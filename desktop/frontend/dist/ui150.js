...
(function(){
  ...
  function renderXRay150(r){
    ...
    const hero='<article class="panel xray-hero150"><div class="panel-title"><div><span class="eyebrow">RAIO X • '+esc(car.municipality||'')+' / '+esc(car.uf||'')+'</span><h3>'+esc(car.car||'')+'</h3><p>'+esc(sicor.scope||'Microdados públicos do SICOR e bases territoriais públicas.')+'</p></div><span class="status-badge '+(operations.length?'ok':'info')+'">'+(sicor.used_cache?'Cache local':'Atualizado')+'</span></div><div class="xray-metrics150">'+
      ...
      '</div><div class="xray-actions150"><button class="btn primary" id="showSicorGlebas150" '+(!s.financed_glebas?'disabled':'')+'>Mostrar glebas financiadas</button><button class="btn ghost" id="exportSicor150" '+(!operations.length?'disabled':'')+'>Exportar operações CSV</button><button class="btn ghost" id="refreshXRay150">Recalcular análise</button><button class="btn ghost" id="openMonitor150">Abrir Monitor MapBiomas</button></div><div class="xray-source150">Fonte principal do histórico individual: microdados públicos do SICOR/BCB. A ausência de operação ou gleba pública não prova inexistência de financiamento. O Monitor do Crédito Rural também informa que sua base representa o universo publicamente rastreável, não todo o crédito rural.</div></article>';
