...
func TestFilterBCBUFYearRowsRequiresExactScope(t *testing.T) {
	rows := []map[string]any{
		{"AnoEmissao":"2026","nomeUF":"MINAS GERAIS","NomeIF":"Banco A","QtdContratos":2.0,"Valor":100.0},
		{"AnoEmissao":"2026","nomeUF":"SÃO PAULO","NomeIF":"Banco B","QtdContratos":3.0,"Valor":200.0},
		{"AnoEmissao":"2025","nomeUF":"MINAS GERAIS","NomeIF":"Banco C","QtdContratos":4.0,"Valor":300.0},
		{"AnoEmissao":"2026","NomeIF":"Sem UF","QtdContratos":9.0,"Valor":999.0},
	}
	got := filterBCBUFYearRows(rows, "MG", "MINAS GERAIS", "2026")
	if len(got) != 1 {
		t.Fatalf("esperava somente MG/2026, obteve %d: %#v", len(got), got)
	}
	if firstNonEmptyStringMapValue(got[0], "NomeIF") != "Banco A" {
		t.Fatalf("linha de UF incorreta: %#v", got[0])
	}
}

func TestAggregateMarketItemsRanksByValue(t *testing.T) {
	items := []BCBMarketItem{
		{Scope:"Município",Kind:"Custeio",Year:"2026",Name:"Café",Contracts:2,Value:100},
		{Scope:"Município",Kind:"Custeio",Year:"2026",Name:"Café",Contracts:3,Value:200},
		{Scope:"Município",Kind:"Custeio",Year:"2026",Name:"Soja",Contracts:1,Value:500},
	}
	got := aggregateMarketItems(items, 10)
	if len(got) != 2 {
		t.Fatalf("esperava 2 produtos agregados, obteve %d: %#v", len(got), got)
	}
	if got[0].Name != "Soja" || got[0].Value != 500 {
		t.Fatalf("ranking inesperado: %#v", got)
	}
	if got[1].Name != "Café" || got[1].Contracts != 5 || got[1].Value != 300 {
		t.Fatalf("agregação de Café incorreta: %#v", got[1])
	}
}

func TestMarketNumberAndStringFallbacks(t *testing.T) {
	row := map[string]any{
		"Nome_Produto":"CAFÉ",
		"QuantidadeContratos":"12",
		"ValorContratos":"1234,50",
	}
	if got := findStringFieldByTokens(row, []string{"PRODUTO"}); got != "CAFÉ" {
		t.Fatalf("produto inesperado: %q", got)
	}
	if got := numberMapValueCandidates(row, []string{"QuantidadeContratos"}, []string{"QTD","QUANT"}); got != 12 {
		t.Fatalf("quantidade inesperada: %v", got)
	}
	if got := numberMapValueCandidates(row, []string{"ValorContratos"}, []string{"VL","VALOR"}); got != 1234.5 {
